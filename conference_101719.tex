...
The runtime of ComE BGMM+VI is equivalent to the runtime of ComE in its big-$O$ notation. ComE's time complexity is $O(\abs{V}\gamma\ell + \abs{V} + T_1 (T_2\abs{V}K + K + \abs{E} + \abs{V}\gamma\ell + \abs{V}K))$, which is linear in time complexity to the graph size: $O(\abs{V} + \abs{E})$ \cite{ComE}.

The node and community embeddings are generated with ComE BGMM+VI in linear time to the graph size: $O(\abs{V}\abs{E})$ (line 1). All friends of each user are determined in $O(E)$ (lines 2-5). A sorted dictionary of the top-$N$ friend suggestions for each user sorted ascending by distances between the node embeddings is computed in $O(\abs{V}^2N))$ (lines 6-13). Top-$N$ friend suggestions are generated for each user ($O(\abs{V})$). For each user, all users not befriended currently are considered ($O(\abs{V})$) and inserted into a sorted dictionary of length $N$ ($O(N)$).

This brings the total time complexity of Algorithm \ref{alg:friend_sug} to $O(\abs{V}\gamma\ell + \abs{V} + T_1 (T_2\abs{V}K + K + \abs{E} + \abs{V}\gamma\ell + \abs{V}K) + \abs{E} + \abs{V}^2N)$. We consider $\gamma$, $\ell$, $T_1$, $T_2$, and $N$ as constant w.r.t the data, therefore the resulting time complexity in big-O notation depends on the graph's size $(\abs{V}, \abs{E})$ and the number of communities $K$: $O(K + \abs{V}K + \abs{E} + \abs{V}^2N)$. The number of communities $K$ can also be considered as constant, in which case the time complexity is $O(\abs{E} + \abs{V}^2N)$.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Ensure{Top-$N$ recommendations for all nodes $R$.}
        % run ComE BGMM+VI
        \State $\Phi, \Phi', \Pi, (\Psi,\Sigma) \leftarrow ComE_{BGMM+VI}(G, K, \gamma, \ell, \zeta, D, m, \alpha, \beta)$ \Comment{Run ComE BGMM+VI.}
        %  generate a set of nodes for each community
        \For{$v \in V$} \Comment{Generate nodes sets $C_k$ per community $k$.}
        \State $k \leftarrow \pi_v$
        \State $C_k \leftarrow C_k \cup \{v\}$
        \EndFor
        % find friends
        \For{$(v, v') \in E$} \Comment{Determine sets of friends for each user.}
        \State $F_v \leftarrow F_v \cup \{v'\}$
        \State $F_{v'} \leftarrow F_{v'} \cup \{v\}$
        \EndFor
        % for each node
        \For{$v \in V$}
        % generate ascending recommendation list for each node
        \State $R_v \leftarrow SortedDict(size=N)$ \Comment{Init recommendations for node $v$.}
        \For{$v' \in C_k \land v' \notin F_v \land v' \neq v$} \Comment{For all non-friends of user $v$.}
        \State $d \leftarrow \|\phi_v - \phi_{v'}\|$ \Comment{Distance of $v$ and $v'$ embeddings.}
        \State $R_v[d] \leftarrow v'$ \Comment{Add $v'$ to $v$'s recommendations at distance $d$.}
        \EndFor
        % get Top-N recommendations for each node
        \State $R_v  \leftarrow R_v.values()$ \Comment{Get top-$N$ recommendations for each node.}
        \EndFor
    \end{algorithmic}
\end{algorithm}
...
The time complexity of Algorithm \ref{alg:friend_sug_com}, when not considering $K$ as a substantial factor in time complexity is equivalent to the time complexity of Algorithm \ref{alg:friend_sug}. Nevertheless, $K$ can substantially reduce the quadratic summand $\abs{V}^2N$ through division by only considering users of the same community for friend suggestions. The reduction in time complexity through Algorithm \ref{alg:friend_sug_com} is only scalable when considering $K$ to be variable with the input dataset's graph size. This assumption can be made when considering communities as broad groups of friends for friend suggestions. With ten times more users, these users form ten times more communities.
