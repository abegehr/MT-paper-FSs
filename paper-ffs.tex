...
\citet{KarateClubDS} is split into training and testing parts. For evaluation, the leave-one-out (LOO) method is used, where for each testing user one edge is left out, then if the left out edge is in the user's top-$N$ recommendations, a hit is counted, otherwise, a miss is recorded.
...
\begin{figure}[htbp]
    ...
    \caption{\citet{KarateClubDS} training dataset node and community embedding and node classification.}
